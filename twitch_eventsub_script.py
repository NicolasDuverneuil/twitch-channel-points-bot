...
CALLBACK_URL = os.getenv("CALLBACK_URL", "https://votre-app.railway.app/webhook")
...
def verify_signature(message_signature, message_timestamp, message_body, secret):
    ...
    hmac_message = message_timestamp + message_body
    signature = hmac.new(
        secret.encode('utf-8'),
...
async def start_webhook_server():
    ...
    # Utilise le port fourni par Railway (ou 8080 en local)
...
async def main():
    ...
    try:
        ...
        # 3. Lister les abonnements existants (optionnel)
        await eventsub.list_subscriptions()
        
        # 4. Créer l'abonnement EventSub
        if not await eventsub.create_eventsub_subscription():
            return
        
        # 5. Démarrer le serveur webhook
...
if __name__ == "__main__":
    ...
    if not CALLBACK_URL or CALLBACK_URL == "https://votre-app.railway.app/webhook":
